...
import com.chenluwei.weike.view.VitamioVideoView;
   ...
   // private GoogleApiClient client;
    ...
    public void onClick(View v) {
        if (v == btnVideoVoice) {//静音按钮
            ...
            isMute = !isMute;
            update(currentVoice);
        } else if (v == btnVideoSwitchPlayer) {//切换播放器(系统和vitamio互换)
            // Handle clicks for btnVideoSwitchPlayer
            AlertDialog.Builder builder = new AlertDialog.Builder(this)
                    .setTitle("提示")
                    .setMessage("当前使用的是万能播放器，如果播放视频不够流畅，请您切换到系统播放器播放")
    ...
    private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SHOW_NETSPEED:
                    String netSpeed = wkUtils.getNetSpeed(VitamioVideoPlayer.this);
                    //显示加载界面的网速
    ...
    private void setPlayData() {//这里注意，从列表传来的是通过path播放，是一个字符串，而从外界文件传来的是一个URI
        if(mediaItems != null && mediaItems.size()>0) {
            ...
            Toast.makeText(VitamioVideoPlayer.this, "没有传入播放地址", Toast.LENGTH_SHORT).show();
        }
        //一进来时就设置一下按钮状态
        setButtonState();
    }
...
//       screenHeight = wm.getDefaultDisplay().getHeight();
    ...
    private void showErrorDialog() {
        new AlertDialog.Builder(this)
                    .setTitle("提醒")
                    .setMessage("视频播放出错，请检查网络或视频是否有损害")
